    ...
    <div className="space-y-1">
      {lessons.map((lesson, index) => (
        <div
          key={lesson.id}
          className={cn(
            "cursor-pointer rounded-lg px-3 py-2 transition-all duration-200 group hover:shadow-sm flex items-center", 
            "min-h-[64px]", // ensures consistent height
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <h4
              className={cn(
                "font-medium text-sm leading-tight truncate", // truncate prevents overflow
                currentLessonId === lesson.id
                  ? "text-primary"
